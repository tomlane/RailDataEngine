using System;
using System.Collections.Generic;
using Microsoft.Practices.Unity;
using Moq;
using NUnit.Framework;
using RailDataEngine.Data.TrainDescriber;
using RailDataEngine.DI;
using RailDataEngine.Domain.Entity.TrainDescriber.Signal;
using RailDataEngine.Domain.Gateway.TrainDescriber;
using RailDataEngine.Gateway.EF;
using RailDataEngine.UnitTests.Common;

namespace RailDataEngine.UnitTests.Gateway.EF
{
    [TestFixture]
    class TTrainDescriberStorageGateway
    {
        [Test]
        public void throws_when_dependencies_are_null()
        {
            var database = new Mock<ITrainDescriberDatabase>();

            Assert.Throws<ArgumentNullException>(() => new TrainDescriberStorageGateway<SignalMessage>(null));
        }

        [Test]
        public void can_be_built_from_static_container()
        {
            var container = ContainerBuilder.Build();
            var gateway = container.Resolve<ITrainDescriberStorageGateway<SignalMessage>>();
            Assert.IsInstanceOf<TrainDescriberStorageGateway<SignalMessage>>(gateway);
        }

        [TestFixture]
        class Create
        {
            [Test]
            public void throws_when_argument_is_null()
            {
                var database = new Mock<ITrainDescriberDatabase>();
                var gateway = new TrainDescriberStorageGateway<SignalMessage>(database.Object);

                Assert.Throws<ArgumentNullException>(() => gateway.Create(null));
            }

            [Test]
            public void calls_save_changes_on_context()
            {
                var database = new Mock<ITrainDescriberDatabase>();
                var context = new Mock<ITrainDescriberContext>();

                var entitySet = new List<SignalMessage>();

                context.Setup(m => m.GetSet<SignalMessage>()).Returns(MockHelpers.BuildMockSet(entitySet).Object);

                database.Setup(m => m.BuildContext()).Returns(context.Object);

                var gateway = new TrainDescriberStorageGateway<SignalMessage>(database.Object);

                gateway.Create(entitySet);

                context.Verify(m => m.SaveChanges(), Times.Once);
            }
        }

        [TestFixture]
        class Read
        {
            [Test]
            public void throws_when_argument_is_null()
            {
                var database = new Mock<ITrainDescriberDatabase>();
                var gateway = new TrainDescriberStorageGateway<SignalMessage>(database.Object);

                Assert.Throws<ArgumentNullException>(() => gateway.Read(null));
            }

            [Test]
            public void returns_expected_result()
            {
                var database = new Mock<ITrainDescriberDatabase>();
                var context = new Mock<ITrainDescriberContext>();

                var entitySet = new List<SignalMessage>
                {
                    new SignalMessage
                    {
                        Id = 5,
                        Data = "train"
                    },
                    new SignalMessage
                    {
                        Id = 7,
                        Data = "supreme"
                    }
                };

                context.Setup(m => m.GetSet<SignalMessage>()).Returns(MockHelpers.BuildMockSet(entitySet).Object);

                database.Setup(m => m.BuildContext()).Returns(context.Object);

                var gateway = new TrainDescriberStorageGateway<SignalMessage>(database.Object);

                var result = gateway.Read(x => x.Id == 5);

                Assert.AreEqual(1, result.Count);
                Assert.AreEqual("train", result[0].Data);
            }
        }

        [TestFixture]
        class Destroy
        {
            [Test]
            public void throws_when_argument_is_null()
            {
                var database = new Mock<ITrainDescriberDatabase>();
                var gateway = new TrainDescriberStorageGateway<SignalMessage>(database.Object);

                Assert.Throws<ArgumentNullException>(() => gateway.Destroy(null));
            }

            [Test]
            public void calls_save_changes_on_context()
            {
                var database = new Mock<ITrainDescriberDatabase>();
                var context = new Mock<ITrainDescriberContext>();

                var entitySet = new List<SignalMessage>();

                context.Setup(m => m.GetSet<SignalMessage>()).Returns(MockHelpers.BuildMockSet(entitySet).Object);

                database.Setup(m => m.BuildContext()).Returns(context.Object);

                var gateway = new TrainDescriberStorageGateway<SignalMessage>(database.Object);

                gateway.Destroy(entitySet);

                context.Verify(m => m.SaveChanges(), Times.Once);
            }
        }
    }
}
